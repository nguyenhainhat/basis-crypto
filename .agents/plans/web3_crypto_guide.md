...
- **The Solution**: By embedding a server-side generated nonce (and an issue timestamp) inside the signed message, the signature is only valid for that _exact_ nonce and _exact_ time window. Once used, the server discards that nonce, making it impossible to replay or reuse the signature.
...
## 5. 🚀 How to Run the Monorepo E2E Locally

You are now ready to run and experience the complete decentralized system locally! Follow Rule 4, here are the exact commands to start your developer server:

*   **Start your local mock blockchain node** (in Terminal A, if Anvil is installed):
    ```bash
    anvil
    ```

*   **Start the NestJS API server** (in Terminal B, running on `http://localhost:3001`):
    ```bash
    yarn workspace @dworkspace/api run start:dev
    ```

*   **Start the Next.js Frontend client** (in Terminal C, running on `http://localhost:3000`):
    ```bash
    yarn workspace @dworkspace/web run dev
    ```

---

## 6. 🛠️ How to Install Anvil & Foundry Toolchain Locally

If you don't have Anvil or the Foundry toolchain installed, follow these steps:

### 📥 Step 1: Install the Foundry Toolchain Installer
Run the official Foundry installation script:
```bash
curl -L https://foundry.paradigm.xyz | bash
```

### ⚙️ Step 2: Load Foundry into your Shell Environment
Once the installer finishes, run this command to temporarily add the Foundry binaries to your current terminal session's PATH:
```bash
export PATH="$PATH:$HOME/.foundry/bin"
```
*(Note: The installer will also add this to your `~/.zshrc` automatically, so future terminal tabs will load it automatically!)*

### ⚡ Step 3: Run foundryup to Download Anvil and Forge
Execute this command to download and install the actual executable binaries:
```bash
foundryup
```

### 🎉 Step 4: Verify & Start Anvil
Now you can successfully start your local developer blockchain node:
```bash
anvil
```

Let me know if you run into any issues running these commands, and I'll help you immediately!
